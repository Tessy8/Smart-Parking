...
lot_occupancy_file = 'lot_occupancy.csv' 
simulation_time_file = "simulation_time.json"
...
def load_data():
    ...
    if os.path.exists(lot_occupancy_file):
        # If the file exists, load the lot occupancy data
...
def advance_time(users_df, bookings_df, lot_occupancy_df):
    ...
    if simulation_time["hour"] >= 24:
        ...
        if simulation_time["day"] > 7:  # Wrap around to Day 1 after Day 7
            simulation_time["day"] = 1
        # Log occupancy for the new day
        # for lot_id, conditions in lot_conditions.items():
        #     lot_occupancy_df = pd.concat(
        #         [
        #             lot_occupancy_df,pd.DataFrame([{"Lot ID": lot_id, "Day": simulation_time["day"], "Current Occupancy": conditions["current_occupancy"]}])],ignore_index=True,)
...
def will_lot_be_available(bookings_df, users_df, lot_occupancy_df, lot_id, day, start_hour, end_hour):
    ...
    # simulated_occupancy = lot_conditions[lot_id]["current_occupancy"]
    global simulated_occupancy
...
def main():
    ...
    load_simulation_time()  # Load the simulation time

    while True:
        print("\n--- Smart Parking Dynamic Pricing System ---")
        print(f"Current Simulation Time: {get_simulation_time()}")
        print("1. Create New User")
        print("2. Add Booking for Existing User")
        print("3. Add Booking for New User")
        print("4. View All Users")
        print("5. View All Bookings")
        print("6. View Lot Details")
        print("7. Exit")
        
        choice = input("Enter your choice (1-7): ").strip()
        
        if choice == '1':
            users_df = create_user(users_df)
            save_data(users_df, bookings_df, lot_occupancy_df)
        elif choice == '2':
            users_df, bookings_df, lot_occupancy_df = add_booking(users_df, bookings_df, lot_occupancy_df)
        elif choice == '3':
            users_df = create_user(users_df)
            user_id = users_df["User ID"].max()
            users_df, bookings_df, lot_occupancy_df = add_booking(users_df, bookings_df, lot_occupancy_df)
            save_data(users_df, bookings_df, lot_occupancy_df)
        elif choice == '4':
            if users_df.empty:
            ...
            if lot_occupancy_df.empty:
                print("No lot details found.")
            else:
                print("\n--- Lot Details ---")
                # Print lot occupancy details including day from lot_occupancy_df
                for _, row in lot_occupancy_df.iterrows():
                    lot_id = row["Lot ID"]
                    day = row["Day"]
                    current_occupancy = row["Current Occupancy"]
                    # Fetching average occupancy from lot_conditions
                    average_occupancy = lot_conditions[lot_id]["average_occupancy"]
                    # Print the details with the day and lot information
                    print(f"Day: {day}, Lot {lot_id}: Current Occupancy = {current_occupancy}, Average Occupancy = {average_occupancy}")
        elif choice == '7':
